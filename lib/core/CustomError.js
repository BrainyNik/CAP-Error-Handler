class CustomError extends Error {
    ...
    constructor({ message, status, code, target, details, module, internal, req }) {
        ...
        if (internal instanceof Error && internal.stack) {
            ...
            const filtered = stackLines.filter(line => !line.includes("normalizeError"));
    ...
    async logError({ cds, tableName }) {
        try {
            if (tableName && cds) {
                ...
                await cds.run(INSERT.into(tableName).entries(payload));
            }
    ...
    /**
     * Convert to CAP-friendly cds.error
     * @param {cds} cds - CAP instance
     * @returns {cds.error}
     */
    toCdsError(cds) {
